...
} from '@content/services/page-interaction';
import {
  pulseAgentConnectionIndicator,
  setAgentIndicatorBottomOffset,
} from '@content/services/agent-connection-indicator';
...
const handleMessage: MessageHandler = (message, _sender, sendResponse) => {
  switch (message.type) {
    ...
    case 'GET_PAGE_CONTENT': {
      ...
      logger.info('GET_PAGE_CONTENT', { usedLength: content.length, preview: content.slice(0, 100) });
    ...
    case 'GET_INTERACTION_SNAPSHOT':
      sendResponse(extractInteractionSnapshot(message.payload?.maxElements, message.payload?.viewportOnly, message.payload?.viewportSegments));
      return;
    case 'SET_INTERACTION_SCROLL':
      sendResponse({ top: setInteractionScrollTop(message.payload.top) });
      return;
    case 'EXECUTE_INTERACTION_ACTION':
      sendResponse(executeInteractionAction(message.payload.action, message.payload.index, message.payload.text));
      return;
    ...
    case 'GET_IFRAME_RECTS': {
      const iframes: Array<{ url: string; rect: { x: number; y: number; width: number; height: number } }> = [];
...
export function useRuntimeMessageBridge() {
  useEffect(() => {
